...
function groupRows(rows: VaccinationRecallRow[]): VaccinationRecallCandidate[] {
  ...
  for (const row of rows) {
    ...
    if (existing) existing.vaccines.push(vaccine);
    else
...
async function loadVaccinationRecallRecipients(
  ...
  const recipients = candidates.map((candidate) => {
    ...
    return evaluateVaccinationRecallRecipient({
      ...
      whatsAppAvailable: isKirimdevConfigured(),
      smsSuppressedPhones,
      quietHours,
...
export async function sendVaccinationRecallReminders(
  ctx: { db: Database; practiceId: string; beforeEmail?: () => Promise<void> },
  patientIds: string[],
) {
  ...
  for (const recipient of recipients) {
    ...
    let deliveredChannel: "whatsapp" | "sms" | "email" = recipient.channel;
    let providerMessageId: string | undefined;
    let delivered = false;

    if (recipient.channel === "whatsapp") {
      const wa = await sendVaccinationReminderWA({
    ...
    } else if (recipient.channel === "sms" && smsSender) {
      try {
        const sms = await sendVaccinationReminderSms({
          to: recipient.clientPhone!,
          patientName: recipient.patientName,
          vaccineName: vaccineNames,
          practiceName: practice.name,
